...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="flex h-full w-[280px] flex-col bg-[#0F172A] text-slate-400 border-r border-slate-800 p-6 relative">
      <div className="flex items-center justify-between px-2 mb-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-black tracking-tight text-white leading-none">ManageMet</h1>
            {profile && <span className="text-[9px] font-black text-primary uppercase tracking-widest mt-1 opacity-80">{profile.role?.name.replace('_', ' ')}</span>}
          </div>
        </div>
            ...
            <div key={group.title} className="space-y-1">
              <button
                ...
                className={clsx(
                  ...
                  hasActiveItem && !isExpanded ? "bg-white/5" : "hover:bg-white/5"
                )}
              >
                <h3 className={clsx(
              ...
              <div className={clsx(
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <item.icon className={clsx("w-5 h-5", isActive ? "text-white" : "text-slate-500 group-hover:text-white")} />
                        {item.name}
                      </div>
      ...
      <div className="mt-auto pt-6 border-t border-slate-100 flex flex-col gap-4">
        {profile && (
          <div className="flex items-center gap-3 px-2">
            <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-xs font-black text-slate-400 uppercase border border-slate-700">
              {profile.full_name[0]}
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] font-black text-white truncate leading-none mb-1">{profile.full_name}</span>
              <span className="text-[9px] font-bold text-slate-500 truncate uppercase tracking-tight">{profile.email}</span>
            </div>
          </div>
        )}
        <button 
          onClick={() => signOut()}
          className="flex items-center gap-3 px-4 py-3 rounded-2xl text-slate-500 text-sm font-black hover:bg-slate-800 hover:text-white transition-all w-full group"
        >
